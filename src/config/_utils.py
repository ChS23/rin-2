...
class UnsetType:
    """Заполнитель для типа Unset.

    Это помогает отличить None от значения по умолчанию
    """
...
def get_config_val(  # noqa: C901, PLR0912, PLR0911
    ...
    Args:
        ...
        default: Значение по умолчанию, если ключ не найден в окружении
        type_hint: Опциональная подсказка типа вместо вывода из `default`

    Raises:
        ValueError: Вызывается, когда значение конфигурации не может быть разобрано.

    Returns:
        Разобранное значение указанного типа
    """

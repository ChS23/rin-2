...
# Числительные прописью. Список нужен полный: первая версия ловила «сто двадцать»,
# но пропускала «два месяца» вместо «полтора» — реальное искажение (02.08.2026).
_NUMERAL_RE = _re.compile(
    r"\b("
    r"од(?:ин|на|ну|ного)|дв(?:а|е|ух)|тр(?:и|ёх|ех)|четыр(?:е|ёх)|пят(?:ь|и)|"
    r"шест(?:ь|и)|сем(?:ь|и)|восем(?:ь|и)|девят(?:ь|и)|десят(?:ь|и|к\w*)|"
    ...
    r"шестьдесят|семьдесят|восемьдесят|девяносто|сто|сотн\w*|двести|триста|"
    r"четыреста|пятьсот|шестьсот|семьсот|восемьсот|девятьсот|тысяч\w*|миллион\w*|"
    r"полтора|полторы|пара|пары"
    r")\b", _re.IGNORECASE)
...
def _reflection_numbers_ok(text: str, facts: list[str]) -> str | None:
    ...
    m = _NUMERAL_RE.search(text or "")
    if m:
        return f"число прописью ('{m.group(0)}') — требуется цифрами и точно из фактов"
    return None

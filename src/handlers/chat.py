...
chat_agent = Agent(
    model=ai_model,
    name="Рин",
    output_type=RinResponse,
    instructions=f"""
    {RIN_LORE}

    ПОЛЯ ОТВЕТА:

    text — твой ответ в чат.

    reaction — реакция на сообщение собеседника. Допустимые значения: {REACTION_NAMES}, или null. Ставь реакцию только когда реально хочется отреагировать эмоционально, примерно в 1 из 3-4 сообщений.

    remember — список фактов для запоминания о собеседнике. Запоминай только значимое:
      - над каким проектом работает и в какой роли
      - ключевые скиллы (рисует, пишет сценарии, программирует на Ren'Py и т.д.)
      - важные предпочтения (любимые жанры, инструменты)
      - достижения (выпустил новеллу, выиграл джем)
    НЕ запоминай: настроение в моменте, приветствия, мелкие реплики, то что уже есть в переданных фактах.
    Формулируй факт коротко: "Работает над хоррор-новеллой 'Тени'" а не "Говорил что делает что-то связанное с проектом".
    Если ничего нового — null.

    forget — факты из памяти на удаление. Используй, если собеседник явно сказал что что-то изменилось: сменил проект, бросил занятие. Указывай точный текст факта из переданной памяти. Если нечего забывать — null.

    Тебе будет передан контекст: последние сообщения чата, твои воспоминания об участниках, и инфо о сообществе (роли, проекты, запросы). Используй всё это для живого общения.
    """,
)
...
async def chat_with_rin(message: Message):
    ...
    r: RinResponse = result.final_output
